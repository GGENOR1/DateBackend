...
class BaseConnectionManager(ABC):
    ...
    @abstractmethod
    async def send_all_mod(self, keys: list[str], bytes_: bytes):
        ...
...
class ConnectionManager(BaseConnectionManager):
    ...
    async def send_all(self, key: str, bytes_: bytes):
        for websocket in self.connections_map[key]:
            print(f"Тут получаем следующее сообщение {bytes_=}")
            await websocket.send_bytes(bytes_)

    async def send_all_mod(self, keys: list[str], bytes_: bytes):
        for key in keys:
            if key in self.connections_map:
                for websocket in self.connections_map[key]:
                    print(f"Отправляем сообщение по ключам {key}: {bytes_=}")
                    await websocket.send_bytes(bytes_)
